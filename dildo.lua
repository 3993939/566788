...
-- Налаштування клавіші відкриття (F4)
...
_G.Smoothness = 0.1
_G.TeamCheck = true
_G.WallCheck = true -- ДОДАНО: Перевірка стін
_G.AimRadius = 500
_G.ShowTracers = true
_G.ShowNames = true
_G.ShowHP = true
...
MainSection:NewSlider("Плавність", "Вища плавність = повільніше наведення", 500, 1, function(s)
...
-- ДОДАНО: Тогл для перевірки стін
MainSection:NewToggle("Перевірка стін", "Не цілитись крізь стіни", function(state)
    _G.WallCheck = state
end)
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("Head") then
            ...
            if hum and hum.Health > 0 then
                ...
                -- ДОДАНО: Перевірка стін
                if IsBehindWall(head) then continue end
...
local function CreateESP(player)
    ...
    connection = RunService.RenderStepped:Connect(function()
        ...
        if _G.EspEnabled then
            ...
            -- Колір залежно від команди
            if player.Team and LocalPlayer.Team then
                if player.Team == LocalPlayer.Team then
                    esp.Box.Color = Color3.fromRGB(0, 255, 100)
                else
                    esp.Box.Color = Color3.fromRGB(255, 50, 50)
                end
            end
...
Library:Notify("Colin Hub", "Натисніть F4 щоб відкрити меню", 5)
